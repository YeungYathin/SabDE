...
class Main:
    ...
    def local_search(self):
        s0 = np.copy(Main.best_individual.vector)
        ...
        s = s0
        fs = self.calculate_value(s)
        for l in range(0, Main.m):
            rand_facility_loc = random.randint(0, Main.m-1)
            s1 = np.copy(s)
            s1[rand_facility_loc] = 1-s1[rand_facility_loc]
            fs1 = self.calculate_value(s1)
            if fs1 < fs:
                s = s1
                fs = fs1
        if fs <= Main.best_individual.value:
            Main.best_individual.vector = s
            Main.best_individual.value = fs
