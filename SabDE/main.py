...
class Main:
    ...
    epsilon = None
    chaotic_var = None
    Fki_success_list = None
    CRki_success_list = None
    strategy_list = None
    ...
    def SabDE(self):
        ...
        while Main.generation < Main.goal_generation:
            ...
            if Main.generation > Main.LP:
                for k in range(0, 6):
                    # Update pk 
                    strategy_success_num_sum = sum(Main.strategy_success_num_list)
                    Main.pk[k] = Main.strategy_success_num_list[k]/strategy_success_num_sum
                    if Main.Fki_success_list[k].getLen() != 0:
                        Main.fk[k] = Main.Fki_success_list[k].get_median()
                    else:
                        Main.fk[k] = random.random() * 0.9 + 0.1
                    if Main.CRki_success_list[k].getLen() != 0:
                        Main.crk[k] = Main.CRki_success_list[k].get_median()
                    else:
            ...
            print(Main.strategy_success_num_list)
    ...
    def initialize(self):
        ...
        Main.strategy_success_num_list = [0, 0, 0, 0, 0, 0]
        
        for index in range(6):
            Main.fk.append(random.random() * 0.9 + 0.1) # Choose fk from (0.1,1)
            Main.crk.append(0.5)
            Main.pk.append(1/6)
            median_heap_1 = MedianHeap()
            Main.Fki_success_list.append(median_heap_1)
            median_heap_2 = MedianHeap()
            Main.CRki_success_list.append(median_heap_2)
        for j in range(0, Main.population_size):
    ...
    def greedy_selection(self, strategy_index):
        ...
        for j in range(Main.population_size):
            ...
            if self.calculate_value(ind.ui) < ind.value:
                ind.vector = ind.ui
                ind.calculate_value()
                Main.Fki_success_list[strategy_index]:MedianHeap.add_number(ind.Fki)
                Main.CRki_success_list[strategy_index]:MedianHeap.add_number(ind.CRki)
                Main.strategy_success_num_list[strategy_index]+=1
...
# To serve the process of getting the median of the corresponding recorded parameters
class MedianHeap:
    def __init__(self):
        self.small_heap = []
        self.large_heap = []
