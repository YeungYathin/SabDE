...
class Main:
        ...
        while Main.generation < Main.goal_generation:
            ...
            if Main.generation > Main.LP:
                for k in range(0, 6):
                    ...
                    if Main.Fki_success_heap[k].getLen() != 0:
                        Main.fk[k] = Main.Fki_success_heap[k].get_median()
                    else:
                        Main.fk[k] = random.random() * 0.9 + 0.1
                    if Main.CRki_success_heap[k].getLen() != 0:
                        Main.crk[k] = Main.CRki_success_heap[k].get_median()
                    else:
    ...
    def greedy_selection(self, strategy_index):
        ...
        for i in range(len(Fki_del_list)):
            Main.Fki_success_heap.remove(Fki_del_list[i])
            Main.CRki_success_heap.remove(CRki_del_list[i])
        ...
        for j in range(Main.population_size):
            ...
            if self.calculate_value(ind.ui) < ind.value:
                ind.vector = ind.ui
                ind.calculate_value()
                Main.Fki_success_heap[strategy_index]:MedianHeap.add_number(ind.Fki)
                Main.Fki_success_list[strategy_index][Main.generation%Main.LP].append(ind.Fki)
                Main.CRki_success_heap[strategy_index]:MedianHeap.add_number(ind.CRki)
